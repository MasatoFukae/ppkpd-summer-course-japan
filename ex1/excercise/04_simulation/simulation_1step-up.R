...
target.addl <- 2 # additional dose of target dose, default is 2
...
d.target <- expand.ev(ID = 1:nsim,
                     ...
                     ii = target.interval) %>%
  mutate(rate = amt) %>%
  mutate(time = target.time)

d0 <- bind_rows(d.step1, d.target) %>%
  arrange(ID, time) %>%
  group_by(ID) %>%
...
sim1 <- mod_est %>%
  env_eval(seed = seed) %>%
  mrgsim(data=d1, end=target.time + target.interval * (target.addl+1), delta=1) %>%
  as.data.frame() %>%
...
write_csv(df, "simulation_1step-up.csv")

# ---- Output to PDF file ----
pdf("simulation_1step-up.pdf", paper = "a4r", width = 20)
